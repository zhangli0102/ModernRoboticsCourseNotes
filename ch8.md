...
where $\frac{\partial \mathfrak{m}}{\partial x}$ plays the same role as the Christoffel symbols of a mass matrix.
...
After choosing the point at mass-weighted/density-weighted centroid of the body as the **center of mass**, we can represent the second derivative of a point mass' position $p(x)$ with respect to time as:

$$
\ddot{p}_i = \dot{v}_b + [\dot{\omega}_b]r_i + [\omega_b]\nu_b + [\omega_b]^2r_i ,
$$

where $r_i$ is the initial position of the point mass. Then, we can represent the body wrench of the whole rigid-body as:

$$
\mathcal{F}_b = \left[\begin{matrix} m_b \\ f_b \end{matrix}\right] = \left[\begin{matrix} \sum_i m_i \\ \sum_i f_i \end{matrix}\right] 
$$

With the help of several properties of the center of mass and skew-symmetric matrix representation, we can simplify the equation as:

$$
\mathcal{F}_b = \left[\begin{matrix} m_b \\ f_b \end{matrix}\right] = \left[\begin{matrix} \mathcal{I}_b\dot{\omega}_b+[\omega_b]\mathcal{I}_b\omega_b \\ \mathfrak{m}(\dot{\nu}_b+[\omega_b]\nu_b) \end{matrix}\right] , 
$$
...
When the inertia matrix is represented in another frame $\{c\}$ that is rotated from $\{b\}$, the inertia matrix can be transferred as:
...
These two transformations can help to calculate the inertia of rigid body consisting of component rigid bodies.
